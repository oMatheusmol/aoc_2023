function day_01(str: string) {
  const total = new Array();
  str.split('\n').forEach((input: string) => {
    let l: number = 0,
      r: number = input.length - 1;

    const arr: Array<string | null> = [null, null];
    while (arr[0] == null || arr[1] == null) {
      if (Number(input[l]) && arr[0] == null) {
        arr[0] = input[l];
      } else {
        l++;
      }
      if (Number(input[r]) && arr[1] == null) {
        arr[1] = input[r];
      } else {
        r--;
      }
  ...
  return total.reduce((acc, curr) => (acc += Number(curr)), 0);
}

const str = `fivepqxlpninevh2xxsnsgg63pbvdnqptmg
